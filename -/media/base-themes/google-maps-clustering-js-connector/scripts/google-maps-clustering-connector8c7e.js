XA.connector.mapsConnector = (function ($, document) {
    ...
    api.loadScript = function(key, callback) {
        ...
        if (!loading) {
            ...
            script.onload = function () {
                //console.log("Google loader has been loaded, waiting for maps api test");
            };
    ...
    api.showMap = function(mapId, options, viewBounds) {
        ...
        if (viewBounds instanceof Array) {
            ...
            if (options.poiCount < 2) {
                idleListener = google.maps.event.addListener(map, "idle", function() { 
                    if (markers[mapId] != undefined && markers[mapId].length > 0 && markers[mapId].length < 2) {
        ...
        zoomListener = google.maps.event.addListener(map, "zoom_changed", function(a, b, c, d) { 
    ...
    api.createClusters = function (mapId) {
        const style = {
            url: '/images/Maps/cluster-icon.png',
            width: 30,
            height: 42,
            textColor: '#99936f',
            textSize: 15,
            lineHeight: 42,
            fontFamily: 'WilliamsCaslonText'
        }

        if (clusters[mapId]) {
            clusters[mapId].clearMarkers();
        }
        clusters[mapId] = new MarkerClusterer(maps[mapId],
            $.map(markers[mapId], function(val, i) { return val.marker; }),
            {
    ...
    api.renderDynamicPoi = function(mapId, data, getPoiContent) {
        ...
        google.maps.event.addListener(marker, "click", function () {
            ...
            else if (typeof (getPoiContent) === "function") {
                var poiId = data.id,
    ...
    api.centerMap = function(mapId, data, centerMap, animate) {
        ...
        // Set the zoom level if the zoom variable is provided
